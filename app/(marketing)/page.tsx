...
const FEATURES = [
  { icon: '📊', title: 'Live KPI Dashboard', desc: 'Real-time KPI cards refreshing every 30s via SSE. Revenue, AOV, return rate at a glance.' },
  { icon: '🤖', title: 'AI Weekly Summary', desc: 'Gemini 2.0 Flash generates plain-English insights: "Revenue up 12% driven by Electronics."' },
  ...
  { icon: '📦', title: 'Inventory Health', desc: 'Turnover velocity, days-of-stock, and low-stock alerts across all 500 SKUs.' },
  { icon: '📁', title: 'ETL Pipeline', desc: 'CSV drag-drop ingest with dedup, type coercion, FK validation, and run logs.' },
];

const STACK = [
  ['Next.js 16', 'Full-stack React framework'],
  ['Neon PostgreSQL', 'Serverless Postgres on free tier'],
  ['Drizzle ORM', 'Type-safe SQL queries'],
  ['Recharts', 'Composable React charts'],
  ['better-auth', 'Google OAuth + RBAC'],
  ['Gemini 2.0 Flash', 'Free-tier AI insights'],
  ...
  return (
    <main className="flex flex-col items-center font-sans">
      ...
      <section className="w-full bg-gradient-to-br from-indigo-900 via-indigo-700 to-purple-600 pt-32 pb-24 text-center text-white px-6">
        <p className="text-indigo-300 text-sm font-medium mb-4 uppercase tracking-widest">Portfolio Project</p>
        ...
        <p className="mt-5 text-lg text-indigo-100 max-w-2xl mx-auto">
          Full-stack analytics app — 50k+ rows of synthetic data, AI insights, cohort analysis, anomaly detection, and real-time KPI refresh.
        </p>
      ...
      <section id="features" className="w-full max-w-6xl px-6 py-20">
        ...
        <p className="text-center text-gray-500 mb-12">Everything built from scratch, all open-source, all free tier.</p>
      ...
      <section className="w-full bg-indigo-600 py-16 text-center text-white px-6">
        <h2 className="text-2xl font-bold">See it live with real data</h2>
        <p className="text-indigo-200 mt-2 mb-6">All charts powered by 50,000+ seeded rows on Neon PostgreSQL.</p>
        <Link href="/sign-in"
      ...
      <footer className="w-full py-6 text-center text-xs text-gray-400 border-t border-gray-200">
        Built by Udit013 · All services on free tier · Open source on GitHub
      </footer>
    </main>
  );
}
